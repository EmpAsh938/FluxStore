...
import 'package:fstore/models/entities/product_badge.dart';
...
class Product {
  ...
  Product.fromJson(Map parsedJson) : id = parsedJson['id'].toString() {
    try {
      ...
      /// Mapping the badges
      if (parsedJson['meta_data'] is List) {
        parsedJson['meta_data']?.forEach((item) {
          // print("_yith_wcbm_badges ${item['key']}");
          if (item['key'] == '_yith_wcbm_badges') {
            // print("key matched");
            // print("item is $item");
            // print("item is ${item['value'][0]['image_url']}");
            badgeData.add(ProductBadge.fromLocalJson(item));
          }
        });
      }
      print("badgeData ${badgeData}" );
      print("badgeData 0 ${badgeData[0]}" );
      print("badgeData 0 value ${badgeData[0].value![0]['image_url']}" );
      // var filterBadge = metaData
      //     .where((item) =>
      // item['key'] == '_yith_wcbm_badges');
      // if (filterBadge.isNotEmpty) {
      //   // isRestricted = filterBadge.firstWhereOrNull(
      //   //         (e) => e['value'] == (loggedInUser?.role ?? '')) ==
      //   //     null;
      //   print("filter badge data is ${filterBadge}");
      //   // print("image index 0 ${filterBadge}");
      //   // print("image id ${filterBadge['id']}");
      //   // print("image key ${filterBadge['key']}");
      //   // print("image value ${filterBadge['value']}");
      //   // print("image url ${filterBadge['value']['image_url']}");
      //   // isRestricted = filterBadge.firstWhereOrNull(
      //   //         (e) => e['value'] == (loggedInUser?.role ?? '')) ==
      //   //     null;
      // }
  ...
  Product.fromShopify(Map json) : id = json['id'] {
    try {
      ...
      var productBadge = json['meta_data'];
      // if (productTags is List) {
      //   for (var item in productTags) {
      //     tags.add(Tag(
      //       id: item,
  ...
  Map<String, dynamic> toManagerJson() {
    ...
    return {
      ...
      'meta_data': badgeData.map((e) => e.value).join(','),
    };
  }
  ...
  bool checkInStock(ProductVariation? productVariation) {
    var inStockProduct =
  ...
  bool? checkVariationProductInStock() {
    if (ServerConfig().isWooType) {
      return (isVariableProduct && (variations?.isNotEmpty ?? false)) ||
          (isVariableProduct == false && true == inStock);
    }
