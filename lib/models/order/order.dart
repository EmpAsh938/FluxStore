...
class Order {
  ...
  Map<String, dynamic> toJson(CartModel cartModel, userId, paid) {
    ...
    try {
      ...
      if (cartModel.selectedStore != null) {
        if (params['meta_data'] == null) {
          params['meta_data'] = [];
        }
        params['meta_data'].add({
          'key': '_shipping_Select_Hakka_Location',
          'value': cartModel.selectedStore?.slug
        });
        params['meta_data'].add({
          'key': '_shipping_Pick_Up_Or_Delivery',
          'value': cartModel.shippingType.rawValue
        });
      }
    } catch (e, trace) {
      printLog(e.toString());
